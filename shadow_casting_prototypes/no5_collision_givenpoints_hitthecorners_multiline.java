...
class no5_collision_givenpoints_hitthecorners_multiline extends JPanel implements Runnable, MouseMotionListener{
	...
	no5_collision_givenpoints_hitthecorners_multiline(){
		...
		for(int r = 0; r < redlines.size(); r++) {
			for(int i = 0; i < 2; i++) {
				...
				for(int j = 0; j < 3; j++) {
					...
					if(j == 0) s_angle = base_angle - 0.1;
					if(j == 1) s_angle = base_angle;
					if(j == 2) s_angle = base_angle + 0.1;
					...
					float denom = (startx - cast_x)*(redlines.get(r).sy - redlines.get(r).ey) - (starty - cast_y)*(redlines.get(r).sx - redlines.get(r).ex);
					
					float t = ( (startx - redlines.get(r).sx)*(redlines.get(r).sy - redlines.get(r).ey) - (starty - redlines.get(r).sy)*(redlines.get(r).sx - redlines.get(r).ex) ) / denom; 
					
					float u = -(((startx - cast_x)*(starty - redlines.get(r).sy) - (starty - cast_y)*(startx - redlines.get(r).sx))/denom);
