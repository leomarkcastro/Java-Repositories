...
class no13_shadow_block_v1_multiclipping extends JPanel implements Runnable, MouseMotionListener, MouseListener, MouseWheelListener{
	...
	no13_shadow_block_v1_multiclipping(){
	...
	public void paint(Graphics g) {
		...
		g2.drawString("Angle: " + lppool.size(), 0, 50);
		...
		*/
		...
		for(int i = 0; i < lppool.size(); i++) {
			g2.drawLine(lppool.get(i).sx, lppool.get(i).sy, lppool.get(i).ex, lppool.get(i).ey);
		}
		
		g2.setColor(Color.blue);
		for(int i = 0; i < lppool.size(); i++) {
			g2.fillOval(lppool.get(i).sx-2, lppool.get(i).sy-2, 4, 4);
			g2.fillOval(lppool.get(i).ex-2, lppool.get(i).ey-2, 4, 4);
		}
		*/
		...
		for(int i = 0; i < intersect.size()-1; i++) {
			...
			shapepath.lineTo(intersect.get(i).ex, intersect.get(i).ey);
			shapepath.lineTo(intersect.get(i+1).ex, intersect.get(i+1).ey);
			shapepath.lineTo(startx, starty);
			shapepath.closePath();
			
			clip.add(new Area(shapepath));
			*/
		...
		/*
		shapepath.moveTo(startx, starty);
		shapepath.lineTo(intersect.get(intersect.size()-1).ex, intersect.get(intersect.size()-1).ey);
		shapepath.lineTo(intersect.get(0).ex, intersect.get(0).ey);
		shapepath.lineTo(startx, starty);
		shapepath.closePath();
		...
		*/
		...
		g2.fillPolygon(points2x, points2y, 3);
		}
		...
		*/
		...
		for(int i = 0; i < intersect.size(); i++) {
			...
			g2.drawLine(startx, starty, intersect.get(i).ex, intersect.get(i).ey);
		}
		}
		...
		*/
		...
		for(int i = 0; i < lppool.size(); i++) {
			g2.setColor(Color.black);
			g2.fillOval(points.get(i).sx-5, points.get(i).sy-5, 10, 10);
		}
		*/
